...
/*
		if(count($segments)> 0 && strpos($segments[count($segments)-1],'.html'))
		{
			$newSegments = array();
			if(count($segments)== 1 )
			{
				if($segments[0]=='news.html')
				{
					$this->set_class('page');
					$this->set_method('news');
					$newSegments[0]='page';
					$newSegments[1]='news';
				}
				else
				...
				$newSegments[0]='page';
				$newSegments[1]='index';
				$newSegments[2]= str_replace('.html','',$segments[0]);
				}
			}
			else if($segments[0]=='products')
			{
				...
				if(strpos($segments[1],'.html'))
				{
					$this->set_method('index');
					$newSegments[1] ='index';
					$newSegments[2]= str_replace('.html','',$segments[1]);
					$segments[1]='index';
				}
				else if($segments[1]=='show')
				{
					$this->set_method('show');
					$newSegments[1] = 'show';
					$newSegments[2]= str_replace('.html','',$segments[2]);
				}
			}
			$this->uri->rsegments = $newSegments;
			return;
		}*/
